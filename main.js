...
let sipkaVlevo = document.querySelector("#sipkaVlevo");
let sipkaVpravo = document.querySelector("#sipkaVpravo");
let sipky = document.querySelector(".sipka");
let obrazek = document.querySelector("#foto");
let pocitadlo = document.querySelector("#pocitadlo");
let aktualniObrazek = 0;
let zacatekPocitadla = 1;
let pocetObrazku = 6;

obrazek.src = "obrazky/"+obrazky[0];
pocitadlo.textContent = zacatekPocitadla+"/"+pocetObrazku;

function klikVpravo(event) {
    aktualniObrazek = aktualniObrazek + 1;
    zacatekPocitadla = zacatekPocitadla + 1;

    if(aktualniObrazek < obrazky.length) {
        obrazek.src = "obrazky/"+ obrazky[aktualniObrazek]
        pocitadlo.textContent = zacatekPocitadla+"/"+pocetObrazku;
    }else {
        aktualniObrazek = 0;
        zacatekPocitadla = 1;
...
sipkaVpravo.addEventListener("click", klikVpravo);
